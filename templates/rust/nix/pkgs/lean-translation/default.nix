...
let
  ...
  translated = stdenvNoCC.mkDerivation {
    ...
    installPhase = ''
      ...
      lean_list=$(mktemp)
      find . -name '*.lean' ! -path '*/proofs/*' -print0 > "$lean_list"

      if [ ! -s "$lean_list" ]; then
        echo "No Lean files generated by Aeneas; expected at least one .lean translation" >&2
        exit 1
      fi

      mkdir -p "$out/lib/lean/Libtemplate"
      aggregator="$out/lib/lean/Libtemplate.lean"
      echo "-- Aggregates all generated Libtemplate modules (generated during nix build)" > "$aggregator"
      ...
      # Preserve the Lake metadata alongside the Lean sources for downstream tooling
